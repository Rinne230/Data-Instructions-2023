...
class gaSVM(ea.Problem):
    def __init__(self, PoolType):
        ...
        ori_data = pd.read_csv("./datas/vertebral/ok3C.csv")

        y = ori_data['tar']
        ori_data = ori_data.drop(columns='tar')
        X = ori_data
        column_minmax_train, column_minmax_test,\
        column_train_labels, column_test_labels \
        = model_selection.train_test_split(X, y, test_size=0.3, random_state = 42)
        print("验证集训练集差异性：")
        print(column_train_labels.value_counts()/len(column_train_labels))
        print(column_test_labels.value_counts()/len(column_test_labels))
        # X_train, X_test, Y_train, Y_test = train_test_split(X, y, test_size=0.1)

        frames_tmp = [column_minmax_test, column_test_labels]
        df_data_tmp = pd.concat(frames_tmp, axis=1)  # 进行数据列合并
        df_data_tmp.to_csv('./datas/vertebral/data_test.csv', index=False)

        self.data = np.array(column_minmax_train)  # 训练集的特征数据（归一化）
        self.dataTarget = np.array(column_train_labels)
        # 设置用多线程还是多进程
    ...
    def test(self, C, G):  # 代入优化后的C、Gamma对测试集进行检验
        data_test = pd.read_csv("./datas/vertebral/data_test.csv")
        X_test = data_test.drop(columns="tar")
        Y_test = data_test["tar"]
        data_test = np.array(X_test)  # 测试集的特征数据（归一化）
        ...
        print("验证集查准率: {:.2f}%".format(precision_score(Y_test, dataTarget_predict, average='macro') * 100))  # 打印验证集查准率
        print("验证集查全率: {:.2f}%".format(recall_score(Y_test, dataTarget_predict, average='macro') * 100))  # 打印验证集查全率
        print("验证集F1值: {:.2f}%".format(f1_score(Y_test, dataTarget_predict, average='macro') * 100))  # 打印验证集F1值
        from sklearn.metrics import confusion_matrix, classification_report
        cm = confusion_matrix(Y_test, dataTarget_predict)
        print("混淆矩阵:")
        print(cm)
        from models import drawCM
        drawCM.ConfusionMatrixPlotter(cm).plot()
        report = classification_report(Y_test, dataTarget_predict)
        print("分类报告:")
        print(report)
