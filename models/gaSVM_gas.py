...
class gaSVM(ea.Problem):
    def __init__(self, PoolType):
        ...
        ori_data = pd.read_csv("./datas/gas/okGas.csv")

        y = ori_data['tar']
        ori_data = ori_data.drop(columns='tar')
        X = ori_data
        gas_minmax_train, gas_minmax_test,\
        gas_train_labels, gas_test_labels \
        = model_selection.train_test_split(X, y, test_size=0.95, random_state = 42)
        print("验证集训练集差异性：")
        print(gas_train_labels.value_counts()/len(gas_train_labels))
        print(gas_test_labels.value_counts()/len(gas_test_labels))
        # X_train, X_test, Y_train, Y_test = train_test_split(X, y, test_size=0.1)

        frames_tmp = [gas_minmax_test, gas_test_labels]
        df_data_tmp = pd.concat(frames_tmp, axis=1)  # 进行数据列合并
        df_data_tmp.to_csv('./datas/gas/data_test.csv', index=False)

        self.data = np.array(gas_minmax_train)  # 训练集的特征数据（归一化）
        self.dataTarget = np.array(gas_train_labels)
        # 设置用多线程还是多进程
    ...
    def test(self, C, G):  # 代入优化后的C、Gamma对测试集进行检验
        data_test = pd.read_csv("./datas/gas/data_test.csv")
